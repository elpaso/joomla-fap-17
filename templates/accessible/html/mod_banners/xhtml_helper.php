<?php
/**
* @version      $Id: xhtml_helper.php 34 2008-03-13 14:39:43Z elpaso $
* @package      Joomla
...
// ABP: override helper too
class modBannersHelperXHTMLStrict
{
    ...
    function renderBanner($params, &$item)
    {
        ...
        if (trim($item->custombannercode))
        {
            ...
            // ABP: clean target for XHTML compatibility
            $html = str_replace('target="_blank"', '', $html);
        }
        else if (BannerHelper::isImage( $item->imageurl ))
        {
            // ABP: changed  border="0"
            $image  = '<img src="images/banners/'.$item->imageurl.'" style="border:none" alt="'.JText::_('Banner').'" />';
            if ($item->clickurl)
            {
                switch ($params->get( 'target', 1 ))
                {
                    ...
                    case 1:
                        ...
                        $a = "<a href=\"$link\" onclick=\"window.open('". $link ."', ''); return false;\">";
                        break;

                    case 2:
                        // open in a popup window
                        $a = "<a href=\"$link\" onclick=\"window.open('". $link ."', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550'); return false;\">";
                        break;
